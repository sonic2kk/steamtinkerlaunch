...
The Menus are all auto-generated from the code.
...
##### GUI Options
An auto-generated Menu which offers all GUI related configuration options.
...
With `CREATETWEAKS` a default template tweak file is auto-generated on game launch, to make creating one a bit easier.
...
The path to those auto-generated tweaks is
...
This is necessary, as aparrently not all auto-generated configs can be tested.
...
Currently, the following options are imported into the Auto Tweak file:
...
#### Proton Selection
**stl** provides the option to easily choose a specific Proton Version per game, by simply selecting one from an auto-generated dropdown Menu in
...
is used as additional source for custom Proton Versions for inclusion into the auto-generated Proton Version (dropdown) List.
...
The menu is auto-generated, by parsing all `CP_*` *(Custom Proton)* URLs from the [Url Config](#Url-Config)
...
The contents of the corresponding categories are auto-generated on the fly *(in `/dev/shm/`)* using [Game Pictures](#Game-Pictures) and [Desktop Files](#Desktop-Files).
With above `auto` both [Game Pictures](#Game-Pictures) and [Desktop Files](#Desktop-Files) are auto-generated/downloaded if missing.
...
Those Desktop-Files are [auto-generated](#Game-Data) using the *(big)* [Game Pictures](#Game-Pictures) in the `STLCFGDIR` and therefore are of no use for generic usage.

#### Game Data

Both [Game Pictures](#Game-Pictures) and [Desktop Files](#Desktop-Files) *(for the [Game Launcher](#Game-Launcher))* are auto-generated/downloaded
...
into `$STLDLDIR/xlive/` and installed into the game directory.
...
When **stl** detects one of those "SE exe" files in the game dir, a requester will pop up with the following options:
...
`ReShade64.dll`, `ReShade32.dll`: renaming not required as they will be placed in the game directory under the required name
...
- `VKBASALT_CONFIG_FILE`: the vkbasalt source config file - it points per default to `STLCFGDIR/vkBasalt.conf` and is auto-generated if not found
The auto-generated `VKBASALT_CONFIG_FILE` points to the files from `RESHADE_DEPTH3D` so it should have been at least checked out once with `CLONE_DEPTH3D`

#### Depth3D
[Depth3D](https://github.com/BlueSkyDefender/Depth3D)
Mostly useful in combination with [SBS-VR](#Side-by-Side-VR).
Set `RESHADE_DEPTH3D` to 1 to install ReShade Depth3D Shader into game directory
...
With `RESHADE_DEPTH3D` enabled `Overwatch.fxh`, `SuperDepth3D.fx`, `SuperDepth3D_VR+.fx` from Depth3D are copied to the game directory.
When the game started just create an initial profile by selecting the autodetected `SuperDepth3D_VR+.fx`
...
- Downloader/Updater of several Shader repositories *(if important/good repos are missing feel free to open an issue)*
  Currently, the following repositories are implemented:
...
where an Archive can easily be picked from an auto-generated Drop-Down Menu.
...
*(might make sense to use a sqlite3 db here, but having a transparent easily simple file structure also has its benefits)*
