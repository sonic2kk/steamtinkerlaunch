...
### Иконка в трее
Когда утилита **stl** использует yad, у нее появляется возможность отображать иконку приложения в трее(системном лотке), которая появится автоматически при запуске **stl** (or the started game) exits.
