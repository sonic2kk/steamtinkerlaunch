...
#### Размер окна GUI
Когда параметр `SAVESETSIZE` активирован во вкладке Глобальные настройки, или [global.conf](#Global-Config) *(включен по-умолчанию)* разрешение будет изменено для всех окон **stl** (как yad, так и zenity) и автоматически. This function requires xwininfo to work.
