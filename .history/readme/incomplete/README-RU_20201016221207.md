...
#### Размер окна GUI
Когда параметр `SAVESETSIZE` активирован во вкладке Глобальные настройки, или [global.conf](#Global-Config) *(включен по-умолчанию)* разрешение будет изменено для всех око **stl** window (both yad and zenity) are automatically saved
