...
Может быть отключена *(включена по умолчанию)* переключением параметра `USETRAYICON` в [Меню настроек](#Меню-Настроек) Global Config Tab or [global.conf](#Global-Config).
